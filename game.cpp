 ...
 * GAME CONSTRUCTOR
 ***************************************/
Game::Game(Point tl, Point br, Point tg)
	: topLeft(tl), bottomRight(br), rifle(br), rifle2(tg)
{
...
void Game :: advanceBird()
{
   ...
   else
   {
      ...
      if (bird->isAlive())
      {
         ...
         {
				Point negPt = bird->getPoint();
				if (negPt.getX() > 200.0)
				{
					negPt.setX(negPt.getX() * -1);
				}

				if (negPt.getY() > 200.0 || negPt.getY() < -200.0)
				{
					negPt.setY(negPt.getY() * -1);
				}
				
            // We have missed the bird, so it loops unless it is a sacred bird
				if (bird->shouldLoop())
				{
					bird->setPoint(negPt);
				}
				else if (!bird->shouldLoop())
				{
					bird->kill();
				}
         }
...
Bird* Game :: createBird()
{
   Bird* newBird = NULL;
	...
	startingPoint.setX(topLeft.getX() - OFF_SCREEN_BORDER_AMOUNT);
	startingPoint.setY(random(-topLeft.getY(), topLeft.getY()));

	int whatBird = random(0, 4);

	switch (whatBird)
	{
	case 0:
		newBird = new Bird(startingPoint);
		break;
	case 1:
		newBird = new ToughBird(startingPoint);
		break;
	case 2:
		newBird = new SacredBird(startingPoint);
		break;
	case 3:
		newBird = new Lander(startingPoint);
		break;
	}
